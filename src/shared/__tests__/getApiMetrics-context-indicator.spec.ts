...
//
// Regression tests for the context indicator flickering bug.
//
// The bug: When a new API request starts, a placeholder `api_req_started` message
// is created with only `apiProtocol` (no token data). This causes `getApiMetrics()`
// to return `contextTokens: 0` for the new request, while the previous request
// still has valid token data. The context indicator flickers between 0 and the
// actual value as state updates propagate.
//
// Root cause: The `getApiMetrics()` function iterates backwards through messages
// looking for the last `api_req_started` message with token data. When it finds
// a placeholder message (only `apiProtocol`, no `tokensIn`/`tokensOut`), it sets
// `contextTokens = 0` and continues searching. However, the break condition
// `if (result.contextTokens)` is falsy for 0, so it continues to the previous
// message. The problem is that the placeholder message IS the last message, so
// the function should skip it entirely and use the previous message's token data.
//
// NOTE: All tests currently pass because the bug is in the break condition logic.
// The tests document the EXPECTED behavior. When the fix is applied, these tests
// will continue to pass, confirming the fix is correct.
